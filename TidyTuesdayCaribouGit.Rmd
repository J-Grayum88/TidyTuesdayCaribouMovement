...
individuals %>%
  ...
  geom_point(aes(deploy_on_longitude, deploy_on_latitude, size = n, color = study_site)) + 
...
locations_with_gaps <- locations %>%
  ...
  mutate(last_lon = lag(longitude), 
        ...
        lag(timestamp), unit = "hours")),
         km = distHaversine(cbind(longitude, latitude),
        cbind(last_lon, last_lat)) / 1000,
  kph = km /  hours) %>%
  ungroup()
...
locations_with_gaps %>%
  ...
  summarize(avg_speed = median(kph), n = n()) %>%
  ...
  labs(title = "Seasonal trend in caribou speed", y = "Average speed(kph)")
```
